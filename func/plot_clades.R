...
# node_labels: named vector of node labels to be displayed (names indicate node numbers, i.e. higher than Ntip(phy)) or TRUE, which means showing phy$node.label (if available) or FALSE (default), which means node labels are not displayed 
# edge_labels: named vector of edge labels to be displayed (names indicate edge order in phy$edge) or TRUE, which means using phy$node.label (if available), or FALSE (default), which means edge labels are not displayed
# root_label: root label to be added to node_labels (e.g., "100" when indicating bootstrap support)
# anc_labels: logical, whether to show node_labels and edge_labels only for clade ancestors
...
#		strw <- max(strwidth(node_labels, units="inches", cex=1, font=node.text.font) * node.text.cex)
...
# edge labels
	...
	if (is.character(edge_labels)) {
		...
		if (isTRUE(anc_labels)) {
			edges <- intersect(edges, which(group == "stem"))
		}
...
#		strw <- max(strwidth(edge_labels[edges], units="inches", cex=1, font=edge.text.font) * edge.text.cex)
