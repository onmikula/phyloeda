...
# sum_classification
### DESCRIPTION
# summing classification probabilities over crown & stem positions and possibly higher units
### ARGUMENTS
# place: output of classify_sequences
### VALUE
# data frame with summed classification probabilities for each query sequence

sum_classification <- function(place, clades=NULL) {
	if (!is.null(clades)) {
		if (any(place$position == "ancestral")) {
			ancestral <- unique(place$clade[place$position == "ancestral"])
			ancestors <- strsplit(ancestral, "_")
			members <- split(clades[,1], clades[,2])
			for (i in seq_along(ancestral)) {
				incl <- sapply(members, function(x, a) all(a %in% x), a=ancestors[[i]])
				if (any(incl)) {
					clades <- rbind(clades, c(ancestral[i], names(members)[incl]))
				}
			}
		}
		newclade <- clades[,2][match(place$clade, clades[,1])]
		place$clade <- ifelse(is.na(newclade), place$clade, newclade)
	}
	idclade <- paste(place$id, place$clade, sep="-")
	idclade <- match(idclade, unique(idclade))
	sumdf <- do.call(rbind, by(place[,c("probability","n_branch")], idclade, colSums))
	sumdf <- data.frame(place[!duplicated(idclade),c("id","clade","position")], sumdf)
	sumdf$position[sumdf$clade %in% clades[,2]] <- "all"
	return(sumdf)
}
...
### FUNCTION
# get_jplace_invocation
### DESCRIPTION
# extracts software invocation from .jplace file
### ARGUMENTS
# jp: .jplace output of EPA/pplacer
### VALUE
# a character vector whose elements are character strings as they occurred in the software invocation

get_jplace_invocation <- function(jp) {
	jp <- gsub("^[[:blank:]]", "", readLines(jp))
	invocation <- jp[grep("\"invocation\"", jp)]
	if (any(grepl("\\\\", invocation))) {
		invocation <- gsub("\\\\", "/", invocation)
	}
	invocation <- unlist(regmatches(invocation, gregexpr("\\\"[^\\\"]+\\\"", invocation)))
	invocation <- gsub("^[[:blank:]]*\\\"|[[:blank:]]*\\\"$", "", invocation[grep("invocation", invocation) + 1])
	invocation <- unlist(strsplit(invocation, "\\s"))
	return(invocation)
}
