...
get_snps <- function(loci, snps=NULL, pis=FALSE, nalleles=4, toupper=FALSE) {
	...
	if (isTRUE(pis)) {
		snps_pos <- lapply(snps_pos, function(x) list(snp=x$snp[x$pis], pis=x$pis[x$pis], x$nalleles[x$pis]))	
	}
	if (nalleles < 4) {
		snps_pos <- lapply(snps_pos, function(x, n) {nall <- x$nalleles <= n; list(snp=x$snp[nall], pis=x$pis[nall], x$nalleles[nall])}, n=nalleles)	
	}
...
clean_data <- function(loci, remove="empty", numeric=FALSE) {
	...
	loci <- lapply(loci, cleaning, remove=remove)
	if (isFALSE(is_list)) {
		loci <- loci[[1]]
	}
	return(loci)
