...
concatenate <- function(dna, toupper=TRUE, as.DNAbin=FALSE, missing="-") {
	...
	if (!is.null(attr(dna[[1]], "snp"))) {
		snps <- lapply(dna, function(x) cbind(snp=attr(x, "snp"), pis=as.character(attr(x, "pis")), nalleles=attr(x, "nalleles")))
		snps <- cbind(locus=rep(names(dna), sapply(snps, nrow)), do.call(rbind, snps))
	} else {
		snps <- NULL
	}
	if (inherits(dna[[1]], "DNAbin")) {
