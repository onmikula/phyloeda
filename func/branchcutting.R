...
branchcutting <- function(phy, psample, heterosp=NULL, consp=NULL, rescale=TRUE, bw="mdif", kernel="epanechnikov", outgroup=NULL) {
	...
	if (!missing(phy)) {
		if (is.character(phy) & length(phy) == 1) {
			newick <- substr(readLines(phy, n=1), 1, 1) == "("
			if (isTRUE(newick)) {
				phy <- ape::read.tree(phy)
			} else {
...
export_bcut <- function(bcut, file=NULL, return=FALSE, col.names=c("ID", "OTU"), taxonomy=NULL) {
	otus <- bcut$otus
	if (!is.null(taxonomy)) {
	...
	if (!is.null(file)) {
		write.table(otus, file, row.names=FALSE, quote=FALSE, sep="\t")
	}
	if (isTRUE(return)) {
		attr(otus, "notus") <- bcut$notus
		return(otus)
	}
}
...
plot.bcut <- function(bcut, which=c("tree","otus","loss","dens"), edge.width=2, show.tip.label=FALSE, tip.cex=0.7, pt.cex=2, show.legend=FALSE, ...) {
	...
	if (which == "tree") {
		...
		if (isTRUE(show.tip.label)) {
			ntip <- length(bcut$phy$tip.label)
			offset <- 0.25 * mean(bcut$phy$edge.length[bcut$phy$edge[,2] <= ntip])
			xlim[2] <- xlim[2] + offset
			strw <- tip.cex * max(strwidth(bcut$phy$tip.label, units="inches", font=3))
			strw <- strw * 1.08 * diff(xlim) / (par("fin")[2] - sum(par("mai")[c(2,4)]))
			xlim[2] <- xlim[2] + strw
		}
		plot(xy, type="n", bty="n", axes=FALSE, ann=FALSE, xlim=xlim)
		draw_tree(bcut$phy, xy, lwd=edge.width, col=col)
		if (isTRUE(show.tip.label)) {
			xytips <- xy[seq(ntip),]
			xytips[,1] <- xytips[,1] + offset
			text(xytips, labels=bcut$phy$tip.label, cex=tip.cex, font=3, adj=c(0,0.25))
		}
	}
