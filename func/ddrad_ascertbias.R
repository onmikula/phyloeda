...
# pop - data frame with two columns: Ind (individual IDs without allelic specification) & Pop (population/species assignement)
...
ddrad_ascertbias <- function(loci, pop, thinning=1) {
    ...
    if (isTRUE(popanalysis)) {
        ...
        indnam <- setNames(lapply(pop$Ind, grep, x=seqnam), pop$Ind)
	...
	for (i in seq_along(loci)) {
        ...
        if (isTRUE(popanalysis)) {
            ...
            popnam <- pop$Pop[match(ind, pop$Ind)]
