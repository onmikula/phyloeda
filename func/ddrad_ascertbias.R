...
ddrad_ascertbias <- function(loci, pop, thinning=1, allele="_[01]$") {
	...
	result <- list(occupancy=occupancy, mdist=mdist, nalleles=nalleles, nsnps=nsnps[,"snp"], npis=nsnps[,"pis"], heterozygosity=hetzyg, ntaxa=ntaxa, mintra=mintra, thinning=thinning)
	return(result)
}
