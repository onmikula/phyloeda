...
#' @param topology topology to be fixed in MrBayes analysis, specified in any way acceptable to [read_tree].
#' @param interleaved logical, whether to write sequence data in interleaved format
#' @param beagle logical, whether to use beagle for parallelization, default is FALSE (disable it) 
...
write_mrbayes_input <- function(seq, file, part, mcmc=NULL, sumt=NULL, prefix=NULL, priors=NULL, topology=NULL, interleaved=FALSE, beagle=FALSE) {
	...
	if (missing(part)) {
		...
		part <- read_part(part, param=TRUE)
	}
	if (any(part$rates == "R")) {
...
#' 
#' @description
#' Assembles the nexus block of MrBayes commands.
#'
#' @param part either a two column matrix indicating starts and ends of loci in the concatenated aligments
#'  (in .fasta or .nexus files) or a name of partition file formatted according to RAxML or MrBayes convention.
#' @param mcmc a vector of MCMC parameters, named by the parameter names used by MrBayes,
