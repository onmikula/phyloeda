...
#' Reading CLUMPAK output file.
#' 
#' @description
#' Reads CLUMPAK output data file.
#'
#' @param file character string, name of the output file.
#' @returns A matrix with estimated ancestry proportions.
#' @details So far it works only for admixture model with no POPINFO prior.
#' @export

read_clumpak_output <- function(file, ind=NULL) {
	lin <- readLines(file)
	lin <- sub("^[[:blank:]]+", "", lin)
	lin <- gsub("[[:blank:]]+", " ", lin)
	lin <- do.call(rbind, strsplit(lin, "\\s"))
	miss <- as.numeric(gsub("[\\(\\)]", "", lin[,3]))
	if (is.data.frame(ind) & !is.null(attr(ind, "K"))) {
		ind <- rownames(ind)
	} else if (is.character(ind) & length(ind) == 1) {
		ind <- rownames(read_str_output(ind))
	} else if (length(ind) == nrow(lin)) {
		ind <- as.character(ind)
	} else if (!is.null(ind)) {
		warning("'ind' has no acceptable value and was changed to 'NULL'")
		ind <- lin[,2]
	} else {
		ind <- lin[,2]
	}
	
	est <- lin[,(which(lin[1,] == ":")+1):ncol(lin)]
	mode(est) <- "numeric"
	K <- ncol(est)
	rownames(est) <- ind
	colnames(est) <- paste0("Cluster", formatC(seq(K), format="d", flag=0, width=nchar(K)))
