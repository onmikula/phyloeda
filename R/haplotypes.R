...
haplotypes <- function(input, fasta=NULL, txt=NULL, return=TRUE, outgroup=NULL, minpres=1, maxmiss=1) {
	...
	haps <- data.frame(hap=rep(names(haps), sapply(haps, length)), seq=unlist(haps), row.names=NULL)
...
#'   or a name of file with such an assignement. Haplotype names are in the 1st column, sequence names in the 2nd column.
...
#'   and in the 2nd column assignmenet to taxa, based on the correspondence of haplotype names.
#' @export
