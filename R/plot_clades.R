...
#'   with tip labels (1st column) and clade assignment (2nd column). The classification may not be comprehensive.
...
# node & edge labels
	...
	if (isTRUE(missinner) & (any(labnodes) | any(labedges))) {	
		...
		if (any(labnodes)) {
			...
			below <- max(c(below, -labnodexy[,2] + 0.5 * node.lab.size))
			left <- max(c(left, -labnodexy[,1] + 0.5 * node.lab.size))
			above <- max(c(above, labnodexy[,2] + 0.5 * node.lab.size - 1))
		}
		if (any(labedges)) {
