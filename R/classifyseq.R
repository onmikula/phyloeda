...
seqmatches <- function(taxa, haps, na.rm=FALSE, retain=TRUE, col.names) {
	...
	matchseq <- setdiff(haps$seq, taxa[,1])
	if (length(matchseq) > 0) {
		r <- character(length(matchseq))	
		for (i in seq_along(matchseq)) {
			r[i] <- haps$hap[haps$seq == matchseq[i]]
			if (!r[i] %in% taxa[,1]) {
				r[i] <- intersect(taxa[,1], haps$seq[haps$hap == r[i]])[1]
			}
		}
		matching <- match(r, taxa[,1])
		result <- data.frame(seq=matchseq, tax=taxa[matching,2])	
...
findqueryseq <- function(ref, haps) {
	...
	if (is.character(haps) & length(haps) == 1) {
		haps <- setNames(read.delim(haps)[,1:2], c("hap", "seq"))
	} else if (is.list(haps) & !is.data.frame(haps)) {
		haps <- haps$assign
	} else if (is.data.frame(haps)) {
		haps <- setNames(haps[,1:2], c("hap", "seq"))
	}
	haps$query <- TRUE
...
epaformat <- function(taxa, haps, epa, file, return=TRUE, na.rm=FALSE, retain=TRUE, method) {

	taxa <- as.data.frame(taxa)
	delim <- grep("^method", names(taxa), ignore.case=TRUE, value=TRUE)
	if (length(delim) == 1) {
		result <- setNames(data.frame(taxa[,1:2], taxa[,delim]), c("id", "clade", "method"))
	} else if (!missing(method)) {
		result <- setNames(data.frame(taxa[,1:2], method), c("id", "clade", "method"))
	} else {
		result <- setNames(data.frame(taxa[,1:2], "predefined"), c("id", "clade", "method"))
	}
	result$totalprob <- result$probability <- result$position <- NA		
	if (isTRUE(retain)) {
		result <- data.frame(result, taxa[,setdiff(names(taxa)[-c(1,2)], delim)])
	}
	...
	if (!is.null(epa)) {
		...
		if (isTRUE(retain)) {
			extracols <- setdiff(names(result), names(epa))
			for (i in seq_along(extracols)) {
				epa[extracols[i]] <- NA
			}
		}
		result <- rbind(result, epa)
	}
