...
write_str_params <- function(df, outputs=c("main.txt", "extra.txt")) {
	...
	hits <- lapply(df$Arg, grep, x=mainparams)
	mainpar <- which(sapply(hits, length) == 1)
	for (i in mainpar) {
		legend <- sub("^#[[:alnum:][:blank:]]+\\/", "/", mainparams[hits[[i]]])
		mainparams[hits[[i]]] <- paste(sub("[[:blank:]]+", " ", df$Arg[i], fixed=TRUE), "   ", df$Value[i], "   ", legend)
	}
	writeLines(mainparams, con=outputs[1])
	df <- df[-mainpar,,drop=FALSE]
	if (nrow(df) > 0) {
		hits <- lapply(df$Arg, grep, x=extraparams)
		extrapar <- which(sapply(hits, length) == 1)
		for (i in extrapar) {
			legend <- sub("^#[[:alnum:][:blank:]]+\\/", "/", extraparams[hits[[i]]])
			extraparams[hits[[i]]] <- paste(sub("[[:blank:]]+", " ", df$Arg[i], fixed=TRUE), "   ", df$Value[i], "   ", legend)
		}
	}
	writeLines(extraparams, con=outputs[2])	
}
