...
#' @param cex.ind.label size of individual labels.
...
plot_str_output <- function(out, palette, interval=FALSE, ord=TRUE, device, file, width=10, height=5, mai=NULL, show.ind.label=FALSE, side.ind.label=1, ylab="Ancestry proportions", main=NULL, line.axis=0.25, cex.ind.label=1, cex.axis=1, cex.lab=1, cex.main=1, font.lab=1, expr, ...) {
	N <- nrow(out)
	...
	par(mai=mai)
	plot(0, 0, xlim=c(0, nrow(out)), ylim=c(0,1), xaxs="i", yaxs="i", xaxt="n", xlab="", ylab=ylab, main=main, type="n", cex.lab=cex.lab, cex.axis=cex.axis, font.lab=font.lab, cex.main=cex.main)
	for (i in seq(N)) {
	...
	if (isTRUE(show.ind.label)) {
		mtext(text=rownames(out)[ord], side=side.ind.label, line=line.axis, at=seq(0.5, nrow(out)-0.5, by=1), cex=cex.ind.label, las=2, ...)
	}
	if (!missing(expr)) {
