...
#' @param order vector of clade names in the desired bottom-top order.
#' @returns Rotated tree as a `phylo` object.
#' @export

reorder_clades <- function(phy, clades, order) {
	clades <- clades[clades[,1] %in% phy$tip.label,]
	if (missing(order)) {
		order <- unique(clades[,2])
	} else {
		order <- intersect(unique(order), clades[,2])
	}
	tiporder <- unname(unlist(split(clades[,1], clades[,2])[order]))
