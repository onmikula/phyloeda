...
read_part_scheme <- function(x) {
	...
	part <- lapply(part, function(x) grep("[DR]NA", x, invert=TRUE, value=TRUE, ignore.case=TRUE))
...
read_iq_stdout <- function(x, param, name) {
	...
	gamma <- isTRUE(grepl("^g", model[-1]))
	free <- isTRUE(grepl("^r", model[-1]))
	rates <- c("G", "R", "Q")[c(gamma, free, !gamma & !free)]
	ncat <- ifelse(gamma|free, getstring("[[:digit:]]+", model[gamma|free]), NA)
	pinv <- any(model == "i")
	eqfreq <- any(model == "fq") | any(sapply(equalfreqmodels, grepl, x=model))
	optfreq <- any(model == "fo")
