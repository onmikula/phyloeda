...
#' 
#' @description
#' Estimates branch lengths from a sample of trees.
#'
...
#' @param strict logical, whether to estimate branch lengths only from the trees, whose topology is strictly
#'   identical with that of a summary tree.
#' @param ultrametric logical, whether the `trees` are ultrametric. If set to `FALSE` (default), it is actually decided
#'   according to tip-to-root distances in the first of the trees.
#' @param fun character string, name of the function estimating branch lengths (default is `"mean"`).
#' @param thin a proportion of mcmc steps to be retained or a subsampling frequency (if `thin > 1`).
#' @param p a proportion of posterior density to be included in HPD/CPD interval of branch length estimate.
#'   If `p=0`, no interval is estimated. The default is now 0, but in future it may change to 0.95. 
#' @param interval a type of interval to be calculated, either `"HPD"` (highest posterior density, default)
#'   or `"CPD"` (central posterior density).
#' @returns An object of class `phylo` amended by branch lengths estimated from the provided sample of trees.
#' @export

estimate_brlen <- function(phy, trees, strict=FALSE, ultrametric=FALSE, fun="mean", thin=1, p=0.95, interval="HPD") {
	...
	ultrametric <- ultrametric | ifelse(rooted, ape::is.ultrametric(trees[[1]], tol=0.00001, option=1), FALSE)
	if (!rooted | (rooted & !ultrametric)) {
		...
		if (p > 0) {
			interval <- match.fun(toupper(interval))
			phy$edge.length.hpd <-  t(apply(edgelengths, 1, interval, p=p))
		}
	} else {
		...
		if (p > 0) {
			interval <- match.fun(toupper(interval))
  			edgelengths <- nodeheights[phy$edge[,2],] - nodeheights[phy$edge[,1],]
  			phy$edge.length.hpd <-  t(apply(edgelengths, 1, interval, p=p))
		}
