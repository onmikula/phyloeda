...
paint_branches <- function(phy, info, color, bg="black", nested=TRUE) {
	...
	paths <- setNames(vector("list", length(clades)), names(clades))
	n <- sapply(clades, length)
	for (i in which(n == 1)) {
		paths[[i]] <- paste(sort(phy$edge[phy$edge[,2] == clades[[i]],]), collapse="-")
	}
	for (i in which(n > 1)) {
		paths[[i]] <- nodepaths(t(combn(clades[[i]], m=2)), phy=phy)
