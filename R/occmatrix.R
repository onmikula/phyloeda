...
occheatmap <- function(occmat, device, file) {
	...
	if (!is.null(nloci)) {
		layout(matrix(c(1, 1, 0, 2, 2, 3), 2, 3, byrow=TRUE), widths=c(1.42, 4.58, 1), heights=c(0.30, hh - 0.30))
	} else {
	...
	if (!is.null(nloci)) {
		...
		plot(cbind(seq(21), 1), type="n", xlim=c(0.5, 21.5), ylim=c(0,1), axes=FALSE, ann=FALSE, xaxs="i", yaxs="i", bty="n")
		rs <- seq(0, 2 * pi, len=200)
		pts <- 0.95 * scale(cbind(0.5 * cos(rs), 0.5 * sin(rs)), center=TRUE, scale=FALSE)
		for (i in seq(21)) {
			ipts <- (nloci[i] * pts) + (rep(1, 200) %*% t(c(i, 0.5)))
			polygon(ipts, border="black", col="lightgrey", lwd=1)
		}		
	}

	par(mai=c(0.22, 1.42, 0.42, 0.02))
	graphics::image(seq(21), seq(N), t(occmat)[,N:1], zlim=c(1e-08, 1), xlim=c(0.5, 21.5), col=palette, axes=FALSE, ann=FALSE, xaxs="i", yaxs="i", bty="n")
	strw <- max(strwidth(rownames(occmat), units="inches"))
	#cex.row <- ifelse(strw > 1.4, 1.4 / strw, 0.7)
	cex.row <- ifelse(strw > 1.4, 1 / strw, 0.7)
	mtext(text=expression("">=""), side=3, line=1.25, at=-0.5, cex=1.5, las=1)
	...
	mtext(text=colnames(occmat), side=2, line=0.25, at=seq(21) - 0.5, cex=0.7, las=2)
	mtext(text=expression("">=""), side=2, line=0.25, at=21.5, cex=1, las=2)
	mtext(text=expression(""<""), side=2, line=0.25, at=-0.25, cex=1, las=2)
